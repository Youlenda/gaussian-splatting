...
# dataset
dataset_rf = dataset_dict['blender']
train_dataset_rf = dataset_rf(datadir, split='train', is_stack=False)
test_dataset_rf = dataset_rf(datadir, split='test', is_stack=True)
...
# parameters
aabb = train_dataset_rf.scene_bbox.to(device)
...
tensorf = eval(model_name)(aabb, reso_cur, device, density_n_comp=n_lamb_sigma, appearance_n_comp=n_lamb_sh,
                           near_far=train_dataset_rf.near_far, alphaMask_thres=alpha_mask_thre, density_shift=density_shift,
                           distance_scale=distance_scale, step_ratio=step_ratio)

allrays_rf, allrgbs_rf = train_dataset_rf.all_rays, train_dataset_rf.all_rgbs
allrays_rf, allrgbs_rf = tensorf.filtering_rays(allrays_rf, allrgbs_rf, bbox_only=True)
trainingSampler = SimpleSampler(allrays_rf.shape[0], batch_size)
...
for iteration in range(first_iter, n_iterations + 1):
    ...
    ray_train_rf, rgb_train_rf = allrays_rf[ray_idx], allrgbs_rf[ray_idx].to(device)
    rgb_map_rf, _ = OctreeRender_trilinear_fast(ray_train_rf, tensorf, chunk=batch_size, N_samples=nSamples, is_train=True, device=device)

    loss_rf = torch.mean((rgb_map_rf - rgb_train_rf) ** 2) 
    total_loss_rf = loss_rf
    loss_reg_L1_rf = tensorf.density_L1()
    total_loss_rf += L1_reg_weight * loss_reg_L1_rf
    summary_writer_rf.add_scalar('train/reg_l1', loss_reg_L1_rf.detach().item(), global_step=iteration)
    ...
    summary_writer_rf.add_scalar('train/PSNR', PSNRs_rf[-1], global_step=iteration)
    summary_writer_rf.add_scalar('train/mse', loss_rf, global_step=iteration)
    ...
    if iteration in update_AlphaMask_list:
        ...
        if iteration == update_AlphaMask_list[1]:
            ...
            trainingSampler = SimpleSampler(allrgbs_rf.shape[0], batch_size)
    ...
    if iteration % vis_every == vis_every - 1 and N_vis != 0:
        PSNRs_test_rf = evaluation(test_dataset_rf, tensorf, OctreeRender_trilinear_fast, f'{logfolder_rf}/imgs_vis/', N_vis=N_vis, prtx=f'{iteration:06d}_', N_samples=nSamples)
        summary_writer_rf.add_scalar('test/psnr', np.mean(PSNRs_test_rf), global_step=iteration)
...
###############################################################################################################################################################################################
    ...
    with torch.no_grad():
        ...
        if iteration % 10 == 0:
            ...
            f'TensoRF - train_psnr: {float(np.mean(PSNRs_rf)):.2f}, '
            f'test_psnr: {float(np.mean(PSNRs_test_rf)):.2f}, mse: {loss_rf:.4f}')
